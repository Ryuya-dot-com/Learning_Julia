...
# ╔═╡ beefca01-0000-11f1-9a01-000000000001
begin
...
md"""
# NB3: 確率・推論・シミュレーション編の演習ノート

**はじめてのJulia — STEP 3（「乱数・標本抽出・再現性」から「不確かさを論文品質で可視化」まで）の実践編**です。

`# TODO` のセルを書きかえて、下の判定セルが ✅ になったらクリアです。今回は読みこむデータがありません——**乱数で、自分でデータを作る**回だからです。

!!! tip "Pluto の約束ごと"
    Pluto では1つのセルに書ける式は1つです。複数の処理を1セルに書きたいときは `( 式; 式 )` か `begin ... end` で囲みます。今回の課題はどれも1式で書けるように作ってあります。
"""

# ╔═╡ beefca03-0000-11f1-9a01-000000000003
md"""
## 課題1: 再現できる刺激リスト（「乱数・標本抽出・再現性」）

参加者番号 `p` を受けとり、**明示的なRNG `Xoshiro(100 + p)`**を作って、cong / incong 各4試行・計8試行をシャッフルした配列を返す関数 `make_trials` を完成させましょう。

種が参加者番号から決まるので、「参加者2に見せた順序」をいつでも作り直せます。
"""

# ╔═╡ beefca04-0000-11f1-9a01-000000000004
...
# ╔═╡ beefca06-0000-11f1-9a01-000000000006
md"""
...
# ╔═╡ beefca09-0000-11f1-9a01-000000000009
if trials_df === missing
...
# ╔═╡ beefca10-0000-11f1-9a01-000000000010
md"""
## 課題3: 標本サイズとSE（「標本分布」）

平均500・SD50の正規分布から標本を1万回作り、標本平均の経験的SEを返す `empirical_se` を用意しました。`ns = [4, 25, 100]` の各nについてSEを計算し、配列 `ses` を作りましょう。

理論SEは `50 / sqrt(n)` です。経験値が理論値へ近づくか、コードで確かめます。
"""

# ╔═╡ beefca11-0000-11f1-9a01-000000000011
function empirical_se(n; nsim = 10_000)
    rng = Xoshiro(2026)
    d = Normal(500, 50)
    means = [mean(rand(rng, d, n)) for _ in 1:nsim]
    return std(means)
end

# ╔═╡ beefca12-0000-11f1-9a01-000000000012
ns = [4, 25, 100]

# ╔═╡ beefca13-0000-11f1-9a01-000000000013
ses = missing # TODO: ns の各 n について empirical_se(n) を並べた配列に
...
elseif ses isa AbstractVector && length(ses) == 3
    md"🤔 3つの値はできています。`empirical_se(n)` を `ns` の順(4, 25, 100)で呼んでいますか?"
else
    md"🤔 3つの数の配列にしましょう。`[empirical_se(n) for n in ns]` の形です。"
end

# ╔═╡ beefca15-0000-11f1-9a01-000000000015
md"""
...
p4 = missing # TODO: plot(ns, ses) に marker や軸ラベルを添えて
...
md"""
## 課題5: 独立2群の順位検定と効果量（「仮説検定入門」）

controlとtraining各10人の順序得点を用意しました。`rank_test` はtrainingを第1群にしたMann–Whitney U検定です。

p値と、trainingの値がcontrolより大きい経験的確率をNamedTuple `rank_summary` に入れましょう。同順位は0.5勝として数えます。
"""

# ╔═╡ beefca20-0000-11f1-9a01-000000000020
begin
    rank_control = [1.0, 2, 2, 3, 3, 3, 4, 4, 5, 5]
    rank_training = [3.0, 4, 4, 5, 5, 5, 6, 6, 6, 7]
    rank_test = MannWhitneyUTest(rank_training, rank_control)
    rank_pair_score(x, y) = x > y ? 1.0 : x == y ? 0.5 : 0.0
end

# ╔═╡ beefca21-0000-11f1-9a01-000000000021
rank_summary = missing # TODO: (p = ..., superiority = ...)

# ╔═╡ beefca22-0000-11f1-9a01-000000000022
if rank_summary === missing
...
elseif rank_summary isa NamedTuple &&
    ...
    0.855はこの標本内の全100組を、同順位0.5として比較した値です。「母中央値が大きい確率」や因果効果の確率とは解釈しません。群の順序を逆にすれば、優越確率は `1 - 0.855` になります。"""
elseif rank_summary isa NamedTuple
    md"🤔 NamedTupleはできています。`mean([rank_pair_score(x, y) for x in rank_training, y in rank_control])` で全組合せを数えてください。"
else
    md"🤔 `(p = ..., superiority = ...)` の形にまとめましょう。"
end

# ╔═╡ beefca18-0000-11f1-9a01-000000000018
md"""
## 自由課題

1. `Normal(500, 50)` を `Exponential(1)` に変え、標本平均の標準化分布がnとともにどう変わるか見る
2. 「推定と不確かさ」の `mean_ci` を使い、95%区間を2000本作って被覆率を数える
3. 「モンテカルロ・シミュレーション」の検定力推定へMCSEを付ける
4. 2群のデータを生成し、「不確かさを論文品質で可視化」のコードで生データ・平均・95%信頼区間を重ね、棒＋SEの図と比較する
5. 対応差の配列を作り、`SignTest` と `SignedRankTest` のp値・区間を比較する
6. 3群の順序得点へ `KruskalWallisTest` を適用し、有意でも群の組合せは特定できないことを確認する
7. `ApproximatePermutationTest(Xoshiro(2301), x, y, mean, 50_000)` で平均差の置換検定を行い、Mann–Whitneyと問いが違うことを説明する
8. `make_trials` を自分の実験の条件名・試行数に書きかえ、`trials.csv` を保存する

---

次は **STEP 4 / 関連と一般線形モデル編**。推定と不確かさの考え方を、相関と回帰へ広げます。
"""
...
manifest_format = "2.0"
project_hash = "c2271a7300bb199d9f297550b41e7d0e55d1a34a"
