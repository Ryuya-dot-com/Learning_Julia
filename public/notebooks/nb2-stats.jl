...
# ╔═╡ c0ffee01-0000-11f1-9a01-000000000001
begin
...
md"""
# NB2: 経験分布から確率分布への演習ノート

**はじめてのJulia — STEP 2（「記述統計の深掘り」から「確率変数と確率分布」まで）の実践編**です。
...
# ╔═╡ c0ffee03-0000-11f1-9a01-000000000003
df = begin
...
# ╔═╡ c0ffee04-0000-11f1-9a01-000000000004
md"""
## 課題1: 四分位数を出す（「記述統計の深掘り」）

`df.rt` の 25% 点・50% 点・75% 点を、**まとめて1つの配列で**求めましょう。
"""

# ╔═╡ c0ffee05-0000-11f1-9a01-000000000005
q1 = missing # TODO: quantile に [0.25, 0.5, 0.75] を渡す

# ╔═╡ c0ffee06-0000-11f1-9a01-000000000006
if q1 === missing
...
elseif q1 isa Number
    md"🤔 1つだけ返っています。割合を配列 `[0.25, 0.5, 0.75]` で渡すと3つまとめて返ります。"
else
    md"🤔 値が合いません。対象は `df.rt`、割合は `[0.25, 0.5, 0.75]` です。"
end

# ╔═╡ c0ffee07-0000-11f1-9a01-000000000007
md"""
...
p2 = missing # TODO: histogram(df.rt, bins = 6, ...)
...
p3 = missing # TODO: @df df boxplot(...) の形で
...
md"""
## 課題4: 正規分布を作り、確率を尋ねる（「確率変数と確率分布」）

平均500・標準偏差50の正規分布 `d_rt` を用意しました。反応時間が550以下となる理論確率を `cdf` で求め、`p550` に入れましょう。

分布を作る構文は `Normal(平均, 標準偏差)`、累積確率は `cdf(分布, 境界)` です。
"""

# ╔═╡ c0ffee14-0000-11f1-9a01-000000000014
d_rt = Normal(500, 50)

# ╔═╡ c0ffee15-0000-11f1-9a01-000000000015
p550 = missing # TODO: d_rt から550以下となる確率を cdf で求める
...
# ╔═╡ c0ffee17-0000-11f1-9a01-000000000017
md"""
## 課題5: 分布から乱数を発生させる（「確率変数と確率分布」）

明示的なRNG `Xoshiro(2026)` を使い、`d_rt` から2000個を生成した配列 `sim_rt` を作りましょう。

構文は `rand(rng, 分布, 個数)` です。具体的な乱数列ではなく、長さと分布の性質を判定セルで検査します。
"""

# ╔═╡ c0ffee18-0000-11f1-9a01-000000000018
sim_rt = missing # TODO: rand(Xoshiro(2026), d_rt, 2000)

# ╔═╡ c0ffee19-0000-11f1-9a01-000000000019
if sim_rt === missing
...
elseif sim_rt isa AbstractVector && length(sim_rt) == 2000 &&
    ...
    md"🤔 長さ2000の配列ですか? `rand(rng, d, n)` の引数順と、使った分布を確認しましょう。"
end

# ╔═╡ c0ffee21-0000-11f1-9a01-000000000021
md"""
## 課題6: 確率をlog空間と裾APIで安定に計算する（「確率変数と確率分布」）

数学的には同じでも、`log(0.01^200)` は途中の累乗が0へunderflowします。また、標準正規分布の10より右の確率を `1 - cdf(...)` で求めると、cdfが先に1へ丸められます。

NamedTuple `stability_summary` に、素朴な計算と安定な計算を4つ入れて違いを確認しましょう。
"""
...
stability_summary = missing # TODO: (log_product = ..., sum_logs = ..., tail_subtraction = ..., tail_direct = ...)

# ╔═╡ c0ffee24-0000-11f1-9a01-000000000024
if stability_summary === missing
    md"""⏳ 次の4つをNamedTupleにします。

    - `log_product = log(rare_probability^repetitions)`
    - `sum_logs = repetitions * log(rare_probability)`
    - `tail_subtraction = 1 - cdf(standard_normal, 10)`
    - `tail_direct = ccdf(standard_normal, 10)`
    """
elseif stability_summary isa NamedTuple &&
       hasproperty(stability_summary, :log_product) &&
       ...
       isapprox(stability_summary.tail_direct, 7.619853024160498e-24; rtol = 1e-12)
    md"""✅ **正解!** 素朴な計算は `-Inf` と0.0へ情報を失いましたが、log空間では約−921.034、`ccdf` では約7.62×10⁻²⁴を保てました。

    0.0は「真の確率が厳密に0」、`-Inf`は「現象が不可能」という証拠ではありません。途中計算がFloat64の表現範囲や精度を超えていないか確認しましょう。"""
elseif stability_summary isa NamedTuple
    md"🤔 NamedTupleはできています。積より先にlogを取り、右裾は `1-cdf` ではなく `ccdf` で直接求めてください。"
else
...
1. `Bernoulli(0.8)`、`Binomial(20, 0.8)`、`Poisson(2.5)` を作り、それぞれから10個ずつ生成する
2. `quantile(d_rt, 0.95)` で上位5%が始まる境界を求める
...
次は **STEP 3 / 確率・推論・シミュレーション編**。作った分布から標本を繰り返し、推定値の揺れへ進みます。
"""
...
manifest_format = "2.0"
project_hash = "28275c8caa77fe5938effdf7ebffe941bc826945"
