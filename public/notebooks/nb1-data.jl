...
**はじめてのJulia — STEP 1(レッスン11〜15)の実践編**です(自由課題でレッスン16の内容も使えます)。
...
elseif length(sec) == 12 && isapprox(sec[1], 0.5125)
    ...
    md"🤔 おしい。1000で割って、12個の値になっていますか? レッスン11を見返しましょう。"
end
...
slow = missing # TODO: df[条件, :] の形で(レッスン14の「行を絞りこむ」)

# ╔═╡ 85cbdcd8-8c9c-11f1-a1cc-bb3216fa38dc
if slow === missing
    md"⏳ 「rt列が600をこえる」を条件にします。書き方はレッスン14を見返しましょう。"
...
if m === missing
    md"⏳ 「まとめて、要約する」の2段構えです。レッスン15の最初のコード例が手本です。"
...
- `leftjoin(df, info, on = :id)` で2つの表を合体して、`group` ごとの平均反応時間を出す(レッスン15)
- 課題4の表 `m` を `unstack` で横持ちにしてみる(レッスン16)
