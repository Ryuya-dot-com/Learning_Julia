  ...
  pages: [
    {
      ...
      b: [
        ...
        "「Juliaを手元に入れる」でJuliaをインストールしてあれば、この補講の内容はその場で試せます。まだの方は、読むだけでも大丈夫です。",
      ],
    },
    {
    ...
    },
    {
      ...
      b: [
        ...
        "使うには `IJulia` というパッケージを入れて、REPLからノートブックを起動します。`]` でパッケージモードに入り、`add` が終わったら Backspace で `julia>` に戻る——「Juliaを手元に入れる」の `Pluto` のときと同じ手順です。",
      ],
      code: `pkg> add IJulia
julia> using IJulia
julia> notebook()`,
    ...
    },
    {
      ...
      b: [
        ...
        "`REPL` — ちょっと試す・パッケージを入れる(「Juliaを手元に入れる」)。すべての土台です。",
  ...
  ex: [
    ...
    },
    {
      ...
      ans: 0,
      why: "続編の演習ノートブックは `Pluto.jl` で配布します。「Juliaを手元に入れる」で起動したあの環境です。パッケージ情報がファイルに埋めこまれるので、開くだけで同じ環境が再現されます。",
      hint: "「Juliaを手元に入れる」の最後に起動した、あのノートブックです。",
    },
  ],
};
