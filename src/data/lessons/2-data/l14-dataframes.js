  ...
  pages: [
    {
      ...
      b: [
        ...
        "使うパッケージは `CSV.jl` と `DataFrames.jl` の2つ。レッスン10の手順で `] add CSV DataFrames` と入れておきましょう(Excelファイルは `XLSX.jl` という別パッケージで読めます)。",
...
─────┼────────────────────────────────────
   ...
   3 │ P01      incong     560.4     true`,
      a: [
        "参加者ID、条件、反応時間、正誤の4列が読みこまれました。2行目に出ている `String3` や `Float64` は各列の型です——レッスン3の型の知識がここで生きています。`size(df)` と打てば `(12, 4)`(12行4列)が返ります。",
      ],
    },
    {
      t: "列を選ぶ: select と「:列名」",
...
─────┼──────────────────
   ...
   3 │ P01        560.4`,
    ...
    },
    {
      t: "行を絞りこむ",
      b: [
        "「600ミリ秒をこえた試行だけ」のような行の絞りこみは、レッスン11のドット記法と組み合わせて書けます。",
      ],
